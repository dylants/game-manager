...
function getScheduleUrl(sport, team, teamId) {
    ...
    if (sport === "NHL") {
        ...
        return "http://spoton.it/api/schedules/nba-" + team + "/nba-" + team + ".ics";
    } else if (sport === "MLB") {
        currentYear = moment().format("YYYY");
        return "http://mlb.mlb.com/soa/ical/schedule.ics?team_id=" + teamId + "&season=" + currentYear;
    } else {
...
function loadGameValues(scheduleUrl, callback) {
    ...
    ical.fromURL(scheduleUrl, {}, function(err, data) {
        ...
        for (i = 0; i < dataKeys.length; i++) {
            ...
            if (entry && entry.start && entry.summary &&
